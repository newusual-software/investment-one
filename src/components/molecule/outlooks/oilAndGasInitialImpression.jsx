...
export function OilAndGasInitialImpression() {
  ...
  useEffect(() => {
    ...
    const query = `*[_type == "OilAndGasInitialImpressionReport"]{
      pdfs[]{
        title,
  ...
  return (
    <div>
      <ul
        className="flex my-4 flex-col px-4 max-h-[400px] overflow-y-auto
            ...
            <div className="flex gap-3">
              <div>
                ...
                <img
                  src={pdf.imageUrl}
                  alt={`${pdf.title} Image`}
                  className="w-[5.5rem] h-[5.5rem] rounded-md"
                />
              </div>
              <div>
                <h2 className="font-bold font-workSans text-xl text-black">
                  {pdf.title}
                </h2>
              </div>
            </div>
